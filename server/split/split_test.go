...
var _ = Describe("Split", func() {
	It("splits paragraphs into sentences", func() {
		...
		Expect(split.SplitSentences(body)).To(Equal([]string{"Foo.", "Bar.", "Baz.", "Quux.", "Plugh."}))
		...
		Expect(split.SplitSentences(body)).To(Equal([]string{
		...
		Expect(split.SplitSentences(body)).To(Equal([]string{
			"Contacting Leiter, the pair gets the U.S.",
			"Navy to intercept Disco Volante, to engage in an underwater battle, and to recover one of the bombs.",
